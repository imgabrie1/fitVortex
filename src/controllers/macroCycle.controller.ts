...
import patchMacroCycleService from "../services/macroCycle/patchMacroCycle.service";

export const createMacroCycleController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const body = req.body;
...
export const patchMacroCycleController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { id } = req.params;
  ...
  const updatedMacroCycle = await patchMacroCycleService(updatedData, id);

  return res.status(200).json(updatedMacroCycle);
};
...
export const deleteMacroCycleController = async (
  req: Request,
  res: Response
): Promise<Response> => {
...
export const getMacroCycleByIDController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { id } = req.params;
...
export const getAllMacroCycleController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const userID = req.id;
...
export const adjustVolumeController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { macroCycleId } = req.params;
